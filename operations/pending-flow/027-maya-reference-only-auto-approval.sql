-- Permit the native Maya -> GCash Transaction details screen to auto-approve
-- without a printed transaction timestamp. This exception is intentionally
-- narrow: only Completed or Processing Maya screens with an exact amount,
-- receiver, typed/OCR Reference ID and current GCash destination can pass.
-- Existing advisory locks and reference claims continue to enforce single use.
...
-- Keep the mature state-transition functions intact. Replace only their
-- timestamp requirements, and abort if the expected definitions ever drift.
do $migration$
declare
  definition text;
  original text;
begin
  ...
  definition := replace(
    ...
    $old$or coalesce(
     ...
     or nullif(v_verification.extracted_data #>> '{timing,receiptDateTime}', '') is null$old$,
    $new$or (
       not (
         v_verification.extracted_data #>> '{detected,route,sourceProvider}' = 'maya'
         and v_verification.extracted_data #>> '{detected,route,destinationProvider}' = 'gcash'
         and coalesce((v_verification.extracted_data #>> '{detected,route,mayaReferenceOnlyPolicy}')::boolean, false) is true
         and v_verification.extracted_data #>> '{detected,route,mayaStatus}' in ('completed','processing')
         and coalesce((v_verification.extracted_data #>> '{timing,withinWindow}')::boolean, false) is false
         and nullif(v_verification.extracted_data #>> '{timing,receiptDate}', '') is null
         and nullif(v_verification.extracted_data #>> '{timing,receiptTime}', '') is null
         and nullif(v_verification.extracted_data #>> '{timing,receiptDateTime}', '') is null
       )
       and (
         coalesce((v_verification.extracted_data #>> '{timing,withinWindow}')::boolean, false) is not true
         or nullif(v_verification.extracted_data #>> '{timing,receiptDate}', '') is null
         or nullif(v_verification.extracted_data #>> '{timing,receiptTime}', '') is null
  ...
  if definition = original
    or position('mayaReferenceOnlyPolicy' in definition) = 0
  then
    raise exception 'Unexpected route auto-approval function definition';
  end if;
  execute definition;

  select pg_get_functiondef(
    'public.finish_picklestreet_balance_receipt_attempt(uuid,uuid,jsonb,text[],text,numeric,boolean,text,jsonb)'::regprocedure
  ) into definition;
  original := definition;
  definition := replace(
    definition,
    $old$or coalesce((data#>>'{timing,withinWindow}')::boolean,false) is not true
   or nullif(data#>>'{timing,receiptDate}','') is null or nullif(data#>>'{timing,receiptTime}','') is null$old$,
    $new$or (
     not (
  ...
  if definition = original
    or position('mayaReferenceOnlyPolicy' in definition) = 0
  then
    raise exception 'Unexpected balance receipt function definition';
  end if;
  execute definition;
end;
$migration$;
...
set instructions = 'In Maya, use Bank Transfer to GCash. Enter the Reference ID and upload the full Transaction details screen. Completed or Processing can auto-verify when the recipient, amount and Reference ID match; each Reference ID can be used only once.',
