...
begin
 -- New Maya evidence binds to the independently configured receiving account.
 ...
 if source is null or source not in('gcash','bdopay','maya','bpi','gotyme','maribank') or jsonb_typeof(route) is distinct from 'object'
   ...
   or route->>'routeId' is distinct from (case when source='maya' then 'maya_configured_receiver' else source||'_to_gcash' end) or route->>'destinationProvider' is distinct from (case when source='maya' then 'maya' else 'gcash' end)
   or route->>'destinationMethodCode' is distinct from (case when source='maya' then 'maya' else 'gcash' end)
   or route->>'parserVersion' is distinct from (case when source='maya' then 'maya_configured_receiver_v1' when source='gcash' then 'gcash_v1' else source||'_to_gcash_v1' end)
...
begin
  ...
  if public.picklestreet_source_provider(v_payment_method) not in('gcash','bdopay','maya','bpi','gotyme','maribank')
    ...
    or v_verification.extracted_data#>>'{detected,route,destinationProvider}' is distinct from (case when v_payment_method='maya' then 'maya' else 'gcash' end)
...
begin
 ...
 if source is null or source not in('gcash','bdopay','maya','bpi','gotyme','maribank')
   ...
   or route->>'routeId' is distinct from (case when source='maya' then 'maya_configured_receiver' else source||'_to_gcash' end)
   or route->>'destinationProvider' is distinct from (case when source='maya' then 'maya' else 'gcash' end)
   or route->>'destinationMethodCode' is distinct from (case when source='maya' then 'maya' else 'gcash' end)
   or route->>'parserVersion' is distinct from (case when source='maya' then 'maya_configured_receiver_v1' when source='gcash' then 'gcash_v1' else source||'_to_gcash_v1' end)
   or route->'sourceMatched' is distinct from 'true'::jsonb
   or route->'destinationMatched' is distinct from 'true'::jsonb
...
;
CREATE OR REPLACE FUNCTION public.picklestreet_receipt_route_config_current(p_method text, p_snapshot jsonb)
 RETURNS boolean
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
begin
 ...
 if lower(p_method)='maya' then
  select * into dest from public.tenant_payment_methods where tenant_id=t and method_code='maya' and is_active
   and char_length(btrim(coalesce(account_name,'')))>=2 and char_length(btrim(coalesce(account_reference,'')))>=3 for share;
  return found and p_snapshot=jsonb_build_object('method',p_method,'name',dest.account_name,'account',dest.account_reference);
 end if;
