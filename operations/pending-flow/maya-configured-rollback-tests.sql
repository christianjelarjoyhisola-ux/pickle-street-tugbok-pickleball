...
update public.tenant_payment_methods set is_active=true,account_name='SYNTHETIC VENUE',account_reference='09170000001'
 where tenant_id='f19f457a-68e2-42ea-9f8e-1f6e8ac84b3a' and method_code='maya';
...
create function pg_temp.ps_receiver() returns jsonb language sql as $$
 ...
 where tenant_id='f19f457a-68e2-42ea-9f8e-1f6e8ac84b3a' and method_code='maya' and is_active
$$;
...
begin
 ...
 case when action='retry' then null else hash end,'maya',ref);
end;$$;
...
begin
 ...
 where tenant_id='f19f457a-68e2-42ea-9f8e-1f6e8ac84b3a' and method_code='maya' and is_active;
 if not receiver_ok then receiver:=jsonb_build_object('method','maya','name','Wrong Receiver','account','00000000000');end if;
...
create temporary table maya_results(name text primary key,passed boolean);
...
create function pg_temp.ps_route_snapshot(method text) returns jsonb language sql as $$
 select jsonb_build_object('method',method,'name',m.account_name,'account',m.account_reference)
 from public.tenant_payment_methods m where m.tenant_id='f19f457a-68e2-42ea-9f8e-1f6e8ac84b3a' and m.method_code='maya';
$$;
...
create function pg_temp.ps_route_data(j jsonb,secondary text default null) returns jsonb language sql as $$
 select jsonb_build_object('schemaVersion',2,'provider','google_vision','feature','DOCUMENT_TEXT_DETECTION','ocrCharacterCount',180,
  ...
  'detected',jsonb_build_object('paymentReference',j->>'submittedReference','route',jsonb_build_object(
   'schemaVersion',1,'routeId','maya_configured_receiver','sourceProvider',public.picklestreet_source_provider(j->>'paymentMethod'),
   'destinationProvider','maya','destinationMethodCode','maya','parserVersion',case when j->>'paymentMethod'='maya' then 'maya_configured_receiver_v1' else public.picklestreet_source_provider(j->>'paymentMethod')||'_to_maya_v1' end,
   'sourceMatched',true,'destinationMatched',true,'recipientMatched',true,'referenceMatched',true,'successMatched',true,
   'secondaryReferences',case when false then '[]'::jsonb else jsonb_build_array(jsonb_build_object('kind',case
     ...
     when public.picklestreet_source_provider(j->>'paymentMethod')='bpi' then 'bpi_transaction' else 'instapay' end,'value',coalesce(secondary,right(j->>'submittedReference',6)))) end)),
...
begin
 ...
 j:=public.begin_picklestreet_receipt_attempt(b,'upload',k,'f19f457a-68e2-42ea-9f8e-1f6e8ac84b3a/receipts/'||b||'/'||k||'.png',repeat('a',64),'maya',null);
 ...
 r:=public.finish_picklestreet_receipt_attempt((j->>'attemptId')::uuid,(j->>'leaseToken')::uuid,d,array['auto_approval_eligible'],ref,0.99,true,null,pg_temp.ps_route_snapshot('maya'));
 if r->>'bookingStatus' is distinct from 'confirmed' then raise exception 'Receipt-only approval failed %',r;end if;
 insert into maya_results values('No typed reference: matching receipt confirms booking',true);
 b:=pg_temp.ps_booking(44002);k:=extensions.gen_random_uuid();
 j:=public.begin_picklestreet_receipt_attempt(b,'upload',k,'f19f457a-68e2-42ea-9f8e-1f6e8ac84b3a/receipts/'||b||'/'||k||'.png',repeat('b',64),'maya',null);
 d:=pg_temp.ps_route_data(j||jsonb_build_object('submittedReference',ref));
 r:=public.finish_picklestreet_receipt_attempt((j->>'attemptId')::uuid,(j->>'leaseToken')::uuid,d,array['auto_approval_eligible'],ref,0.99,true,null,pg_temp.ps_route_snapshot('maya'));
 ...
 insert into maya_results values('OCR duplicate reference rejects entire booking without typed entry',true);
 b:=pg_temp.ps_booking(44003);k:=extensions.gen_random_uuid();
 j:=public.begin_picklestreet_receipt_attempt(b,'upload',k,'f19f457a-68e2-42ea-9f8e-1f6e8ac84b3a/receipts/'||b||'/'||k||'.png',repeat('c',64),'maya',null);
 r:=public.finish_picklestreet_receipt_attempt((j->>'attemptId')::uuid,(j->>'leaseToken')::uuid,pg_temp.ps_route_data(j),array['payment_reference_unverified'],null,0.99,false,null,pg_temp.ps_route_snapshot('maya'));
 if r->>'bookingStatus'='confirmed' then raise exception 'Unreadable receipt autoapproved';end if;
 insert into maya_results values('Unreadable reference saved for review',true);
end;$$;
select * from maya_results;
...
begin
 ...
 d:=pg_temp.ps_route_data(j||jsonb_build_object('submittedReference',ref));
 r:=public.finish_picklestreet_balance_receipt_attempt((j->>'attemptId')::uuid,(j->>'leaseToken')::uuid,d,array['auto_approval_eligible'],ref,0.99,true,null,pg_temp.ps_route_snapshot('maya'));
 if r->>'balanceStatus' is distinct from 'settled' then raise exception 'Receipt-only balance did not settle %',r;end if;
 insert into maya_results values('Additional-payment receipt confirms without typed reference',true);
end;$$;
