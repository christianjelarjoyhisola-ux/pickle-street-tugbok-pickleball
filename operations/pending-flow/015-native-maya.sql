...
begin
 ...
 if p_patch ? 'paymentMethods' then
   ...
   for method in select value from jsonb_array_elements(p_patch->'paymentMethods') loop
     ...
     if method->>'methodCode' in('bdo_pay','bdo','bdopay','bpi','gotyme','maribank') then
 ...
 if downstream ? 'paymentMethods' then
   downstream:=jsonb_set(downstream,'{paymentMethods}',(select coalesce(jsonb_agg(
     case when item->>'methodCode' in('bdo_pay','bdo','bdopay','bpi','gotyme','maribank')
 ...
 -- Keep a disabled Maya upload target available before its account is configured.
...
-- Clear only the old inherited receiver, never an independently configured Maya account.
update public.tenant_payment_methods maya set is_active=false,account_name=null,account_reference=null,
 instructions='Send from Maya to the Maya account shown. Upload your completed Maya receipt and transaction reference.'
where maya.tenant_id='f19f457a-68e2-42ea-9f8e-1f6e8ac84b3a' and maya.method_code='maya'
 and maya.qr_image_url is null and maya.qr_storage_path is null
 and exists(select 1 from public.tenant_payment_methods gcash where gcash.tenant_id=maya.tenant_id and gcash.method_code='gcash'
  and gcash.account_name=maya.account_name and gcash.account_reference=maya.account_reference);
update public.tenants set updated_at=clock_timestamp() where id='f19f457a-68e2-42ea-9f8e-1f6e8ac84b3a';
