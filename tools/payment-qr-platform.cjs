...
create temp table qr_functions as select oid,md5(pg_get_functiondef(oid)) fingerprint from pg_proc where pronamespace='public'::regnamespace and prokind='f' and proname<>'save_picklestreet_payment_settings';
`;
const verify=`
do $$ begin
...
(async()=>{
 ...
 const sql=read('014-shared-payment-qr.sql'),hash=crypto.createHash('sha256').update(sql).digest('hex');
 const base=sql.replace(/^begin;$/m,()=>"begin isolation level repeatable read;\nset local lock_timeout='5s';\n"+checks).replace(/commit;\s*$/i,'');
 if(mode==='apply'){
   const v=JSON.parse(read('payment-qr-validation.json'));if(v.hash!==hash||!v.rolledBack)throw Error('Validate exact migration first');
 }
 const result=await query(base+(mode==='validate'?'\n'+read('payment-qr-rollback-tests.sql'):'')+'\n'+verify+(mode==='validate'?"\nselect * from qr_checks;rollback;":"\nselect true as unrelatedFunctionsAndTenantSettingsPreserved;commit;"));
 const report={at:new Date().toISOString(),hash,rolledBack:mode==='validate',result};
 fs.writeFileSync(path.join(dir,mode==='validate'?'payment-qr-validation.json':'payment-qr-release.json'),JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report));
