...
create temp table qr_functions as select oid,md5(pg_get_functiondef(oid)) fingerprint from pg_proc where pronamespace='public'::regnamespace and prokind='f';
`;
const verify=`
do $$ begin
...
(async()=>{
 ...
 const sql=read('017-owner-booking-actions.sql'),hash=crypto.createHash('sha256').update(sql).digest('hex');
 const base=sql.replace(/^begin;$/m,()=>"begin isolation level repeatable read;\nset local lock_timeout='5s';\n"+checks).replace(/commit;\s*$/i,'');
 if(mode==='apply'){
   const v=JSON.parse(read('owner-booking-actions-validation.json'));if(v.hash!==hash||!v.rolledBack)throw Error('Validate exact migration first');
 }
 const result=await query(base+(mode==='validate'?'\n'+read('owner-booking-actions-rollback-tests.sql'):'')+'\n'+verify+(mode==='validate'?"\nselect * from owner_action_results;rollback;":"\nselect true as unrelatedFunctionsAndTenantSettingsPreserved;commit;"));
 const report={at:new Date().toISOString(),hash,rolledBack:mode==='validate',result};
 fs.writeFileSync(path.join(dir,mode==='validate'?'owner-booking-actions-validation.json':'owner-booking-actions-release.json'),JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report));
