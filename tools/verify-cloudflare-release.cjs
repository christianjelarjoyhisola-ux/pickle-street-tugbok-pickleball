...
async function bootstrap(slug){const r=await fetch(endpoint,{method:'POST',headers:{apikey:key,'Content-Type':'application/json',Origin:origin},body:JSON.stringify({p_tenant_slug:slug,p_hostname:'picklestreet.pages.dev'}),signal:AbortSignal.timeout(20000)});return {status:r.status,data:await r.json()};}
