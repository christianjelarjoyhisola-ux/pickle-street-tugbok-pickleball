...
(async () => {
  ...
  try {
    ...
    await page.locator('[data-entry-policy-back]').click();
    ...
    assert.equal(await page.evaluate(() => window.entryFinished), false);
    passed.push('Back returns to welcome without completing entry');
    await policyOpen();
    await page.keyboard.press('Escape');
    assert.equal(await page.locator('#psWelcome').evaluate(el => el.matches(':modal')), true);
    assert.equal(await page.evaluate(() => window.entryFinished), false);
    passed.push('Escape returns to welcome without completing entry');
    await policyOpen();
