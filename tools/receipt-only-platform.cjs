...
create temp table qr_functions as select oid,md5(pg_get_functiondef(oid)) fingerprint from pg_proc where pronamespace='public'::regnamespace and prokind='f' and proname not in('create_picklestreet_provisional_hold','create_picklestreet_group_hold','apply_picklestreet_group_reschedule','picklestreet_receipt_route_ready','auto_approve_picklestreet_receipt_route','finish_picklestreet_balance_receipt_attempt','reject_picklestreet_duplicate');
`;
const verify=`
do $$ begin
...
(async()=>{
 ...
 const sql=read('016-receipt-only-checkout.sql'),hash=crypto.createHash('sha256').update(sql).digest('hex');
 const base=sql.replace(/^begin;$/m,()=>"begin isolation level repeatable read;\nset local lock_timeout='5s';\n"+checks).replace(/commit;\s*$/i,'');
 if(mode==='apply'){
   const v=JSON.parse(read('receipt-only-validation.json'));if(v.hash!==hash||!v.rolledBack)throw Error('Validate exact migration first');
 }
 const result=await query(base+(mode==='validate'?'\n'+read('receipt-only-rollback-tests.sql'):'')+'\n'+verify+(mode==='validate'?"\nselect * from ps_route_results;rollback;":"\nselect true as unrelatedFunctionsAndTenantSettingsPreserved;commit;"));
 const report={at:new Date().toISOString(),hash,rolledBack:mode==='validate',result};
 fs.writeFileSync(path.join(dir,mode==='validate'?'receipt-only-validation.json':'receipt-only-release.json'),JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report));
