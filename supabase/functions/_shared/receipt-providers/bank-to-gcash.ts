...
export function parseBankToGcashReceipt(
  ...
  if (config.primaryReferencePattern && !primary.ambiguous) {
    ...
    if (validReference(value)) {
      ...
      // Do not let a nearby date or other generic label value impersonate a
      // provider-native reference when receipt-only approval depends on it.
      primary.field = {
        value: null,
        raw: null,
        source: "missing",
  ...
  if (
    ...
    // Receipt-only checkout deliberately has no customer-entered reference.
    // A strict provider-native OCR pattern remains independent evidence and
    // is still protected by the primary and rail replay checks.
