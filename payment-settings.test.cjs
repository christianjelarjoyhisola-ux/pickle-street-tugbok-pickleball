...
function form(selected,shared,pnb={}) {
  ...
  const rows=[row('gcash',shared),row('pnb',pnb)];
...
test('shared GCash edits apply to every source without enabling inactive methods or changing PNB',()=>{
  ...
  const result=api.collect(api.methods(original),form(['gcash','maya','pnb'],updated,original[2]));
  ...
  assert.equal(result.find(r=>r.code==='gotyme').isActive,false);assert.equal(result.find(r=>r.code==='maya').isActive,true);
...
test('removing the shared QR clears it for every source while retaining its payment instructions',()=>{
  ...
  assert.equal(result.find(r=>r.code==='gcash').instructions,'Venue payment reminder');assert.match(result.find(r=>r.code==='maya').instructions,/not the InstaPay/);
});
