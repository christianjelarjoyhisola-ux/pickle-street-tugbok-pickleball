(function(){
  ...
  const hours=value=>`${Number((Number(value)/60).toFixed(2))} ${Number(value)===60?'hour':'hours'}`;
  ...
  function renderManager(result){
    ...
    el.querySelector('[data-credit-issue]')?.addEventListener('click',()=>{
      const input=el.querySelector('#weatherCreditHours');
      if(!input.reportValidity())return;
      const minutes=Math.round(Number(input.value)*60);
      if(minutes<1||Math.abs(minutes-Number(input.value)*60)>0.001){message("Enter hours such as 1, 1.5, or 2.");return;}
      ...
      saveManager('issue',{minutes,reason:el.querySelector('#weatherCreditReason').value});
    });
  }
